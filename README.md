...
![firestore-access-rules](/pictures/rules.PNG "Profile Information Query Rules")
...
As a small aside, we have included a completely private firestore collection *reviewAuthors*. *ReviewAuthors* is used for repudiation purposes. Every time a review is written, our server notes stores the review's unique ID and the writer's unique ID in this collection. Not only does this allow us to easily find a review's author, but when clients query for review data we do not expose any identifying information about the author should the review be written anonymously. 
